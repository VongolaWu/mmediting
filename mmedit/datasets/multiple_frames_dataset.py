...
class MultipleFramesDataset(BasicFramesDataset):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        self.num_overlapped = num_overlapped
        super().__init__(ann_file, metainfo, data_root, data_prefix, pipeline,
                         test_mode, filename_tmpl, search_key,
                         file_client_args, depth, num_input_frames,
                         num_output_frames, fixed_seq_len, load_frames_list,
                         **kwargs)
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for folder in folders:
            num_data = (self.seq_lens[folder] - num_frames_each_data) // (
                num_frames_each_data - self.num_overlapped) + 1
            first_index = [
                i * (num_frames_each_data - self.num_overlapped)
